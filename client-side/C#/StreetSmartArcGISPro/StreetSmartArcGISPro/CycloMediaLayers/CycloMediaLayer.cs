    ...
    private async Task CreateFeatureClassAsync(ArcGISProject project, string fcName, SpatialReference spatialReference)
    {
      ...
      await QueuedTask.Run(() =>
      {
        ...
        using (var geodatabase = new Geodatabase(pathLocation))
        {
          ...
          {
            EventLog.Write(EventLogLevel.Warning, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (CreateFeatureClassAsync) error: {e}");
    ...
    public async Task<EditOperation> SaveFeatureMembersAsync(FeatureCollection featureCollection, Envelope envelope)
    {
      return await QueuedTask.Run(() =>
      {
        ...
        if (featureCollection != null && featureCollection.NumberOfFeatures >= 1)
        {
          ...
          EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (SaveFeatureMembersAsync) Start writing number of recordings: {recordings?.Length ?? 0}");
          ...
          if (Layer != null && recordings != null)
          {
            ...
            foreach (Recording recording in recordings)
            {
              ...
              if (!exists.ContainsKey((string)recording.FieldToItem(idField)))
              {
                if (Filter(recording))
                {
                  EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (SaveFeatureMembersAsync) Start writing recording to database: {recording.ImageId}");
                  ...
                  EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (SaveFeatureMembersAsync) Finished writing recording to database: {recording.ImageId}");
                }
              }
              else
              {
            ...
            {
              EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (SaveFeatureMembersAsync) delete element from database: {row.Value}, {row.Key}");
        ...
        if (MapView != null && Layer != null && _cycloMediaGroupLayer != null && _addData == null)
        {
              ...
              Math.Abs(extent.YMax - _lastextent.YMax) > epsilon)
          {
            ...
            if (thisEnvelope != null)
            {
              ...
              {
                EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (OnMapViewCameraChanged) loaded features, start saving: {_addData.NumberOfFeatures}");
                EditOperation editOperation = await SaveFeatureMembersAsync(_addData, thisEnvelope);

                if (editOperation.IsEmpty)
                {
                  EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (OnMapViewCameraChanged) there are no features to write to the layer");
                }
                else
                {
                  bool value = await editOperation.ExecuteAsync();
                  EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (OnMapViewCameraChanged) finished writing features, result: {value}");

                  if (value == false)
                  {
                    string errorMessage = editOperation.ErrorMessage;
                    EventLog.Write(EventLogLevel.Information, $"Street Smart: ({nameof(CycloMediaLayer)}.cs) (OnMapViewCameraChanged) the error message is: {errorMessage}");
                  }
                }
              }
